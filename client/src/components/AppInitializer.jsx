...
const AppInitializer = ({ children }) => {
  ...
  useEffect(() => {
    ...
    updateReducer(addProjects, memo.projects.length ? { projects: memo.projects } : null, isLoading)
  ...
  useEffect(() => {
    ...
    updateReducer(addSkills, memo.skills.length ? { skills: memo.skills } : null, isLoading)
  }, [memo.skills, skills.isFetching, skills.isPending, dispatch])

  // 🔹 Commit 5 — Education
  useEffect(() => {
    const isLoading = education.isFetching || education.isPending
    updateReducer(addEduc, memo.education.length ? { education: memo.education } : null, isLoading)
  ...
  }, [memo.testimonials, testimonial.isFetching, testimonial.isPending, dispatch])

  // 🔹 Commit 8 — Pricing Plans
  useEffect(() => {
    const isLoading = plans.isFetching || plans.isPending
    updateReducer(addPlan, memo.plans.length ? { plans: memo.plans } : null, isLoading)
  ...
  useEffect(() => {
    ...
    updateReducer(addServices, memo.services.length ? { services: memo.services } : null, isLoading)
  }, [memo.services, services.isFetching, services.isPending, dispatch])

  // 🔹 Commit 11 — FAQs
  useEffect(() => {
    const isLoading = faqs.isFetching || faqs.isPending
    updateReducer(addFAQs, memo.faqs.length ? { faqs: memo.faqs } : null, isLoading)
