...
import axios from 'axios' // GeoIP API ke liye
...
// Helper to get country and city from IP (free API)
...
export const addVisitors = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, pages_visited FROM visitors WHERE ip_address = ? AND user_agent = ? LIMIT 1`,
      [ipAddress, userAgent]
    )

    if (rows.length > 0) {
      const visitorId = rows[0].id
      // Append page and update info
      await pool.query(
        `UPDATE visitors
      ...
      await pool.query(
        `INSERT INTO visitors (
          ...
          first_visit_at,
          last_visit_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, JSON_ARRAY(JSON_OBJECT('page', ?, 'visited_at', NOW())), NOW(), NOW())`,
        [ipAddress, userAgent, browser, os, deviceType, referrer, country, city, page]
      )
    }
